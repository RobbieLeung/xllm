...
void Batch::process_sample_output(const RawForwardOutput& raw_output,
  ...
  if (raw_output.outputs.size() != output_targets_.size()) {
    LOG(WARNING) << "Batch output count mismatch, expected "
                 << output_targets_.size() << " targets but got "
                 << raw_output.outputs.size() << " outputs";
  }
...
void Batch::process_sample_output(const SampleOutput& sample_output,
  ...
  if (num_outputs != static_cast<int64_t>(output_targets_.size())) {
    LOG(WARNING) << "Batch output count mismatch, expected "
                 << output_targets_.size() << " targets but got "
                 << num_outputs << " outputs";
  }
