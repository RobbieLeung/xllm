...
runtime::Options MakeTestRuntimeOptions(int64_t max_seqs_per_batch) {
...
ModelInputParams MakeMultiSequenceDecodeParams(const torch::Device& device) {
  ModelInputParams p;
  p.batch_forward_type = BatchForwardType::DECODE;
  p.num_sequences = 2;
  p.kv_max_seq_len = 9;
...
TEST(CudaGraphExecutorTest, DecodeMetadataFastPathUpdatesPersistentBuffers) {
  ...
  xllm::layer::flashinfer::FlashinferWorkspace::get_instance().initialize(
      device);
  ModelArgs args = MakeTestModelArgs();
  runtime::Options options = MakeTestRuntimeOptions(/*max_seqs_per_batch=*/2);
  ...
  ModelInputParams params = MakeMultiSequenceDecodeParams(device);
...
TEST(CudaGraphExecutorTest, DecodeMetadataFastPathFallbackMatchesLegacyPath) {
  ...
  torch::Tensor tokens = torch::tensor({10, 11}, iopt);
  torch::Tensor positions = torch::tensor({20, 21}, iopt);
  ModelInputParams fast_params = MakeMultiSequenceDecodeParams(device);
  ModelInputParams fallback_params = MakeMultiSequenceDecodeParams(device);
